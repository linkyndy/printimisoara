...
class RoutesController extends AppController {
	...
	public $uses = array('Route', 'Station');
...
/**
 * admin_configure method
 *
 * @return void
 */
	public function admin_configure($method = 'fetch') {
		if(!in_array($method, array('fetch', 'discover'))){
			$this->Session->setFlash('Metoda invalida.', 'error');
			$this->redirect(array('action' => 'configure'));	
		}
		$this->set('method', $method);
	}
		
/**
 * admin_discover method
 *
 * @return void
 */
	public function admin_fetch() {
		if($this->request->is('post')){
			if($data = $this->Route->fetch($this->request->data)){
				$this->set('data', $data);
			} else {
				$this->Session->setFlash('Rutele nu au putut fi gasite.', 'error');
				$this->redirect(array('action' => 'configure'));
			}
		}
	}
	
	public function admin_discover() {
		if($this->request->is('post')){
			if($data = $this->Route->discover($this->request->data)){
				$this->set('data', $data);
			} else {
				$this->Session->setFlash('Rutele nu au putut fi calculate.', 'error');
				$this->redirect(array('action' => 'configure'));
			}
		}
	}
	...
	public function admin_compute(){
		if($this->Route->compute()){	
			$this->Session->setFlash('Baza de date a fost generata cu succes!', 'success');
			$this->redirect(array('controller' => 'users', 'action' => 'dashboard'));
		}
		$this->Session->setFlash('Baza de date nu a putut fi generata. Verifica log-ul pentru mai multe detalii', 'error');
		$this->redirect(array('controller' => 'users', 'action' => 'dashboard'));
	}
